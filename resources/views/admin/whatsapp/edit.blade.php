    ...
    <div class="p-4 sm:ml-64 bg-white text-gray-900">
        <div class="mt-14">
            <div class="mb-3 flex justify-between items-center">
                ...
                <h1 class="text-xl font-semibold">Edit Nomor WhatsApp</h1>
            </div>

            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <span class="text-red-500">Catatan: pastikan nomor whatsapp menggunkan format 62 (contoh: 62878xxxxxxxx)</span>
                    <form action="{{ route('admin.whatsapp.update', $phone->id) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PATCH')
                        <div class="grid grid-cols-1 gap-4">
                            <div>
                                <label for="whatsapp" class="block mb-2 text-sm font-medium">Nomor WhatsApp<span class="text-red-500">*</span></label>
                                <input id="whatsapp" type="text" name="whatsapp" class="w-full border-gray-300 rounded-md" value="{{ !old('whatsapp') ? $phone->whatsapp : old('whatsapp') }}" required>
